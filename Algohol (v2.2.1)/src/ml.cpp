...
void regressions::SimLinReg::add(float x, float y)
{
	...
	if(n==0) {head = m;}
	else if(n>0) {trav -> next = m; m -> prev = trav; m -> next = NULL;}
...
void regressions::SimLinReg::extract(char *file)
{
	...
	if (fptr==NULL) {fptr = fopen(file, "w");}

	else if (fptr!=NULL)
	{
		printf("\nYour file already exists, choose one option:\n");
		printf("\n1. Append data (a)\n");
		printf("2. Overwrite data (o)\n");
		printf("\nChoice (a/o): "); scanf("%c", &over);

		if (over=='a') {fptr = fopen(file, "a"); printf("\nAppending to existing file...\n\n");}
		else if (over=='o') {fptr = fopen(file, "w"); printf("\nData overwritten!\n\n");}
		else {perror("Unknown input error!\n");}
	}

	else {perror("File pointer misbehaving!\n");}
	...
	printf("(%f,%f) extracted to \"%s\"\n\n", trav -> x, trav -> y, file);
	printf("Data extracted to \"%s\" successfully!\n\n", file);
